...
public class JbossStructuredPackage {
	...
	private void createWorkSpace(){
		...
		try{
			...
			backupWorkdir.mkdirs()
		}
		catch (Exception e){
			throw new Exception("[E] JbossStructuredPackage.createWorkSpace Impossible to create backupWorkdir"+e.getMessage())
		}
		try{
			templateEngineWorkdir.mkdirs()
		}
		catch (Exception e){
			throw new Exception("[E] JbossStructuredPackage.createWorkSpace Impossible to create templateEngineWorkdir "+e.getMessage())
		}
	...
	private void checkStructureOfPackage(){
		...
		println "[D] pkgExternalConfigRemoteInstanceList"+pkgExternalConfigRemoteInstanceList
		println "[D] pkgExternalLibList"+pkgExternalLibList
		println "[D] pkgExternalLibRemoteInstanceList"+pkgExternalLibRemoteInstanceList
	...
	private void checkRemoteInstanceDirectory(){
		pkgExternalConfigRemoteInstanceList.each{
			def RemoteInstanceDirectory rid=it
		...
		pkgExternalLibRemoteInstanceList.each{
			def RemoteInstanceDirectory rid=it
			def name=rid.getInstName()
			if(rid.control()!= 0){
				throw new Exception("[E] checkRemoteInstanceDirectory the RemoteInstanceDirectory $name isn't available ")
   ...
   def cr=t.run()
	...
	private void deployGroupofApplicationsForEachInstance(){
		
	}
	private void deployLibDirectoryForEachInstance(){
		
		println "[I] deploy Lib Directory on Each Instance"
		def dl=new File(templateEngineWorkdir,"LibMapping")
		...
		jbossInstanceList.each{
			...
			for (def i=0;i<pkgExternalLibList.size();i++){
				...
				def RemoteInstanceDirectory rid=pkgExternalLibRemoteInstanceList.get(i)
				def ridName=rid.getInstName()
				if(rid.syncFromLocalDirectoryToRemote(dm.absolutePath)!= 0){
	...
	private void deployConfigDirectoryForEachInstance(){
		println "[I] deploy Config Directory on Each Instance"
		def dc=new File(templateEngineWorkdir,"ConfMapping")
		...
		jbossInstanceList.each{
			...
			for (def i=0;i<pkgExternalConfigList.size();i++){
				...
				def RemoteInstanceDirectory rid=pkgExternalConfigRemoteInstanceList.get(i)
				def ridName=rid.getInstName()
				if(rid.syncFromLocalDirectoryToRemote(dm.absolutePath)!= 0){
					throw new Exception ("[E] deployConfigDirectoryForEachInstance sync of Remote Instance Directory $ridName : Failed")
				}
	...
	public Integer deployPackage(){
		...
		prepareDeployPackage()
		//   deploy ear/war
		deployGroupofApplicationsForEachInstance()
		//   deploy Config
		deployConfigDirectoryForEachInstance()
		//   deploy Lib
		deployLibDirectoryForEachInstance()
		//     => restore backup
		JbossStructuredPackage.restorePackage()
	}
