...
static def Integer statusOfOneApplicationOnScope(def Jboss7Shell js,def String deployName, def String[] scope){
	...
	def AppGroupList=[]
	def AppInstList=[]
	// liste des deployment présent dans chaque server-group du scope
	scope.each{ group ->
		def command="/server-group="+group+":read-children-resources(child-type=deployment)"
	...
	if (AppGroupList){
		...
		def command="/:read-children-resources(child-type=host)"
		def resultCmd=js.runCLICommand(command)
		if (resultCmd.isSuccess()){
...
statusOfOneApplicationOnScope(js,"SubAppliName1-v1.2.war","main-server-group,server-group-name1")
